...
For a class to justify its existence, it needs to have two distinct features:
  ...
  3. State -
A class must have some kind of state that defines the attributes of its instances. In the case of a simple rectangle, this could simply be its length and breadth. In the case of a book it can be number of pages, an author, genre etc...
